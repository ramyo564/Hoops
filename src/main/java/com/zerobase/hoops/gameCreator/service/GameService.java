...
public class GameService {
  ...
  public CreateResponse createGame(CreateRequest request, String token)
    ...
    // 이메일로 유저 조회
    String email = this.tokenProvider.parseClaims(token.substring(7))
        .getSubject();
  ...
  public UpdateResponse updateGame(UpdateRequest request, String token)
    ...
    var game =
        this.gameRepository.findByGameIdAndDeletedDateTimeNull(
                request.getGameId())
  ...
  public DeleteResponse delete(DeleteRequest request, String token)
    ...
    var game = this.gameRepository.findByGameIdAndDeletedDateTimeNull(
            request.getGameId())
        .orElseThrow(() -> new CustomException(GAME_NOT_FOUND));
    ...
    if (creatorFlag) {
      Long gameCreateCount =
          this.gameRepository.countByDeletedDateTimeNullAndUserEntityUserId(
                  user.getUserId())
              .orElse(0L);
